...
MODEL_PATH="Qwen/Qwen3-4B"  # Default model path
...
# Load CUDA 12.8 toolkit so nvcc is available for flashinfer JIT compilation
module load cuda/12.8.1 2>/dev/null || echo "Warning: cuda/12.8.1 module not found, flashinfer JIT may fail"
...
# Use Flash Attention 2 backend if nvcc is unavailable (flashinfer requires nvcc for JIT)
if ! command -v nvcc &>/dev/null; then
    echo "nvcc not found — falling back to FLASH_ATTN backend (bypasses flashinfer JIT)"
    export VLLM_ATTENTION_BACKEND=FLASH_ATTN
fi
